...
export const fetchAllInfluencesData = async (countries: string[]): Promise<Record<string, InfluenceData[]>> => {
  ...
  try {
    ...
    await Promise.all(countries.map(async (country) => {
      try {
        ...
        if (error) {
          ...
          result[country] = sampleData[country] || [];
          return;
        }
    ...
    if (Object.keys(result).length === 0) {
      ...
      return countries.reduce((acc, country) => {
        acc[country] = sampleData[country] || [];
        return acc;
...
export const useAllInfluencesData = (countries: string[]) => {
  return useQuery({
