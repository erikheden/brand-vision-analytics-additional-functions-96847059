...
const SelectionPanel = ({
  ...
  const getUniquePreferredBrands = () => {
    ...
    brands.filter(item => item.Brand && typeof item.Brand === 'string').forEach(item => {
    ...
    const preferredBrandNames = Array.from(normalizedBrands.entries()).map(([normalizedName, variations]) => getPreferredBrandName(variations, normalizedName)).filter(Boolean).sort();
    return preferredBrandNames;
  };
  const uniqueBrandNames = getUniquePreferredBrands();
  return <Card className="p-6 bg-white border-2 border-[#34502b]/20 shadow-lg rounded-xl transition-all duration-300 hover:shadow-xl">
      <div className="space-y-6">
        <div className="flex flex-col space-y-2">
          <CountrySelect selectedCountry={selectedCountry} countries={countries} onCountryChange={country => {
          console.log("Country changed to:", country);
          setSelectedCountry(country);
          setSelectedBrands([]);
          setSelectedIndustry("");
        }} />
          {!selectedCountry && <div className="flex items-center justify-center mt-2 text-[#34502b] animate-bounce">
        ...
        {selectedCountry && <>
            <IndustryTags industries={industries} selectedIndustry={selectedIndustry} onIndustryToggle={industry => {
          console.log("Industry toggled:", industry);
          handleIndustryToggle(industry, brands);
        }} />
            <BrandSelection brands={uniqueBrandNames} selectedBrands={selectedBrands} selectedCountries={[selectedCountry]} onBrandToggle={handleBrandToggle} onBatchToggle={handleBatchToggle} onClearBrands={handleClearBrands} />
          </>}
      </div>
    </Card>;
};
